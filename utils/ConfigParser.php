...
class ConfigParser
{
    ...
    const PDO_MANDATORY_KEY = ["HOST", "DB", "USER", "PWD", "PORT", "DRIVER"];
    const LDAP_MANDATORY_KEY = ["HOST", "PWD", "USER", "DN"];

    public static function parse_config_file($path, $section, $classname)
    {
        if (!file_exists($path)) {
        ...
        switch ($ext) {
            case "json":
                if (mime_content_type($path) === "application/json") {
                    return self::parse_json($path, $section, $classname);
                }
                throw new JsonException("Config file is not a json file or the JSON syntaxe is invalide");
                break;
            case "ini":
                return self::parse_ini($path, $section, $classname);
                break;
            default:
    ...
    public static function parse_ini($path, $section, $classname): stdClass
    {
        if ($section === null) {
        ...
        $keys = $classname === "ConnectionDB" ? self::PDO_MANDATORY_KEY : self::LDAP_MANDATORY_KEY;
        ...
        return $classname === "ConnectionDB" ? self::make_pdo_config($raw_conf) : self::make_ldap_config($raw_conf);
    }

    public static function parse_json($path, $section, $classname): stdClass
    {
